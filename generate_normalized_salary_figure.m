...
% rent-adjusted normalized GSR salary at SIO from January 2015 to May
...
cpi = zeros(size(t));
cpi_table = readtable('cpi_western_urban_jan2024.xlsx');
...
hold off;
text(datetime(2023,1,15),0.89,'Contract wages article takes effect','rotation',90,'fontsize',11);
text(datetime(2024,1,15),1.0,'Settlement','rotation',90,'fontsize',11);
ylim(ylimits);
...
zori_table = readtable('zori_unadjusted_all_homes_plus_multifamily_by_city_jan2024.csv');
zori_san_diego_raw = table2array(zori_table(10,9:end))';
zori_time = (datetime(2015,01,01):calmonths(1):datetime(2024,1,1))'; 
...
set(f,'position',[360 42 760 740]);
exportgraphics(f,'combined_normalized_gsr_salary.pdf');